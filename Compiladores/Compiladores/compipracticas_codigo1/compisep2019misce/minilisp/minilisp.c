...
static Obj *apply_func(void *root, Obj **env, Obj **fn, Obj **args) {
    ...
    puts("fn_funcion_arg_antes");
    print(*args);
    puts("fn_funcion_args_despues");
    *params = (*fn)->params;
    puts("params");
    print(*params);
    *newenv = (*fn)->env;
...
static Obj *apply(void *root, Obj **env, Obj **fn, Obj **args) {
    ...
    if ((*fn)->type == TFUNCTION) {
        ...
        *eargs = eval_list(root, env, args);
        printf(" @[EEARGS "); print(*args); printf(" ]EEARGS@ ");
...
static Obj *eval(void *root, Obj **env, Obj **obj) {
    printf(" @eval_sexp tipo [ %d]", (*obj)->type);
    printf(" @eval_sexp[ "); print(*obj); printf(" ]eval_sexp@ ");
    switch ((*obj)->type) {
    case TINT:
    case TPRIMITIVE:
    case TFUNCTION:
    ...
    case TSYMBOL: {
        // Variable
        printf(" @eval_obj name <%s>", (*obj)->name);
        Obj *bind = find(env, *obj);
        printf(" @symbol[ "); print(bind); printf(" ]symbol@ ");
        if (!bind)
    ...
    case TCELL: {
        ...
        *expanded = macroexpand(root, env, obj);
        printf(" @EXPAND[ "); print(*expanded); printf(" ]EXPAND@ ");
        if (*expanded != *obj)
            return eval(root, env, expanded);
        *fn = (*obj)->car;
        printf(" @[FN "); print(*fn); printf(" ]FN@ ");
        *fn = eval(root, env, fn);
        printf(" @[FNEVA "); print(*fn); printf(" ]FNEVA@ ");
        *args = (*obj)->cdr;
        printf(" @[ARGS "); print(*args); printf(" ]ARGS@ ");

...
//Linea
	...
	exp:    VAR {		$$ = new ParserVal(machine.agregarOperacion("varPush_Eval")); 
		...
		|nombreProc '(' arglist ')' { $$ = new ParserVal(machine.
agregarOperacionEn("invocar",($1.ival))); machine.agregar(null); System.out.println("Llamada a Procedimiento");} //instrucciones tiene la estructura necesaria para la lista de argumentos
		;//Llamada a procedimiento
		|nombreFunc '(' arglist ')' { $$ = new ParserVal(machine.
agregarOperacionEn("invocar",($1.ival))); machine.agregar(null); System.out.println("Llamada a Funcion");} //instrucciones tiene la estructura necesaria para la lista de argumentos
		;//
	...
	nop:    {$$ = new ParserVal(machine.agregarOperacion("nop"));}
		;
	//Cada que se pinta linea se detiene
